/*************************************************************************
 * uflash.c - BIOS flash ROM utility for Micro8088 board
 *
 *
 * Copyright (C) 2020 Aitor Gomez (spark2k06).
 * Provided for hobbyist use on the Micro8088 board.
 *
 * This program is adapted from Sergey's xiflash.c 
...
void delay(unsigned int delay)
{
	while (delay--) {
		...
		{
			push cx
			mov cx, 0F0h // ~ 1 milisecond at 4.77Mhz
			delay2:
			nop
			loop delay2
			pop cx
